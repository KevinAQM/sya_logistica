...
class FormularioScreen(Screen):
    ...
    def _cargar_materiales_thread(self):
        ...
        try:
            response = requests.get(f"{SERVER_URL}/api/materiales", timeout=10)
    ...
    def mostrar_popup_agregar_material(self):
        ...
        producto_input = TextInput(
            multiline=False,
            font_size=dp(18),
            hint_text="Comience a escribir para buscar..."
        )
    ...
    def _enviar_requerimientos_thread(self, datos, popup):
        ...
        try:
            response = requests.post(
                f"{SERVER_URL}/api/enviar-requerimientos",
                json=datos,
                timeout=30
            )
            response.raise_for_status()
            ...
            Clock.schedule_once(lambda dt: self.mostrar_exito(
                ...
                "Los requerimientos han sido enviados correctamente al servidor."
            ))
